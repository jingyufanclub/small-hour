...
The runtime owns execution within a turn. Optional SQLite components commit local effects with their receipts, preserve model steps and spending, claim and resume application-defined tasks, and hand saved outputs to consumer-supplied sinks. Consuming applications define workflows, facts, authorization, memory policy, eligibility, and transport implementations.
...
- Explicit task execution with versioned ordered steps, durable claims, named concurrency scopes, due times, cancellation and bounded recovery.
- Transactional output staging and recoverable delivery with explicit sink contracts, stable idempotency keys, and separate acceptance and confirmation evidence.

The interface supports text, tool calls, and structured results. Streaming, image input, and audio input are not exposed.

## Usage

Requires Node.js 20.3 or later and an ESM consumer. Install repository dependencies with `npm ci` and build with `npm run build`. Package exports resolve to the generated `dist` directory.
...
## Model-step checkpoints

Import `SqliteModelStepStore` from `small-hour/durable/sqlite`, supply an existing synchronous SQLite connection, and call `initialize()`. `run(request, runtime, input)` binds a scoped, versioned operation contract to a runtime turn and returns `{ attemptId, replayed, result }`.
...
Import `SqliteTaskRunner` from `small-hour/durable/sqlite` and supply an existing synchronous SQLite connection, workflow definitions, and `leaseMs`. Call `initialize()` to create its task, local-receipt, model-step and delivery-evidence tables. Each workflow supplies a `kind`, `version`, ordered `steps`, and synchronous `authorize` policy. Local steps supply transactional `execute` and `parseResult` functions; model steps supply a synchronous `prepare` function returning a configured runtime and turn input.

`enqueue(request, { concurrencyScope, dueAt, maxAttempts })` saves an explicitly submitted task and can participate in an existing local transaction. The outer transaction owns final commit. Each `runNext({ signal })` call claims at most one eligible task. Completed steps reuse their receipts; new steps receive the saved preceding results. Claims fence local commits and task transitions. Named scopes allow one running or uncertain task at a time. No timer or polling loop starts automatically.

`inspect({ scope, id })` returns task state, reasons, step results and available model reports. `cancel(key, reason)` records cancellation without undoing committed effects. Unfinished model steps remain uncertain and are never automatically repeated. `resolve(key, { status, reason, evidence })` can close an uncertain task as failed or cancelled after application reconciliation. See [task contracts](docs/tasks.md) for retries, leases and effect boundaries.

## Saved-output delivery

Define a workflow containing one step with `kind: "delivery"` and a consumer-supplied `DeliverySink`. The task's scoped identity and JSON `input` are the immutable output contract, including its destination. Enqueue it in the transaction that commits the originating local effect or product. Dispatch requires a committed task and uses the existing runner's claims, concurrency scopes and attempt limits.

A sink supplies `idempotency: "key" | "none"`, `send(product, context)`, and optional `reconcile(product, context)`. The context includes a stable scoped `idempotencyKey`, optional cancellation signal, and `assertActive()`. Authorization is checked before new sink work. Sink outcomes distinguish `deferred`, `rejected`, `uncertain`, `accepted`, and `confirmed`; definite outcomes require evidence. Provider acceptance does not establish device or user receipt.

`runNext()` handles queued, due and expired-claim work. `retryDelivery(key)` explicitly attempts recovery of an uncertain handoff under the sink's original recovery contract. Unknown sends require sink-guaranteed idempotency or authoritative reconciliation. Delivery workflows cannot include model or local steps, so transport recovery cannot repeat creation. `recordDeliveryOutcome(key, { attemptId, outcome })` records application-verified evidence against an existing attempt, including later receipt confirmation. Inspection preserves task status and delivery evidence separately.

Transport implementations, timeouts, idempotency guarantees, receipt verification and disclosure policy remain consumer responsibilities. See [delivery contracts](docs/delivery.md).
...
Core turn reports are in-memory snapshots; the optional model-step store persists progress and completed results. Cancellation prevents new work but cannot undo effects or forcibly interrupt implementations that ignore the signal. A completed tool call does not independently establish durable persistence or delivery.
...
- **Application services:** provide the database connection, task submission and wakeups, concurrency scopes, transport implementations, and user interfaces. The optional task runner owns claims, safe local retries, and dispatch of saved outputs when selected. No database server, automatically running worker, scheduler service, or transport service is bundled.
...
Use Node.js 22.13 or later to run `npm run check` for type checking, behavioral tests, and the build. SQLite tests use `node:sqlite`; runtime imports remain compatible with Node.js 20.3 or later and a consumer-supplied SQLite driver. The checks cover runtime, provider, local transaction, model-step restart, durable spending, task recovery and saved-output delivery contracts; live-model behavior and consumer integrations require separate verification.
